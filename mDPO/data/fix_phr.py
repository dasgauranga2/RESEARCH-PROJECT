...
# function when given the masked correct answer
# and a list of replacements for each mask will
# replace the mask tokens with replacements
def create_rejected_answer(correct_answer, replacements):
    
    matches = list(re.finditer(r"<MASK>.*?</MASK>", correct_answer))
    result = ""
    last_index = 0
...
for i in range(len(data)):
    ...
    for (inp, correct_out), (_, hallucinated_out) in zip(correct_pairs, hallucinated_pairs):
        ...
        if not correct_out.strip().lower().startswith(ref_ttp[0][0].strip().lower()):
            replacements.append(ref_ttp[0][0].strip())
        else:
            replacements.append(correct_out.strip())

    rejected_answer = create_rejected_answer(correct_answer_masked, replacements)

    result_data.append({
        "prompt": prompt,
        "chosen": data[i]['correct_answer'],
