...
# PROBLEM: mDPO is less likely to produce the correct response when a zoomed-in or blurred image is given
# HYPOTHESIS: mDPO reduces the likelihood of chosen response when corrupted image is given. 
#             But, if that corruption happens in real-life the model is less likely to generate the correct response. 
#             In mDPO, image is corrupted by randomly cropping an image and resizing to original size i.e. producing a zoomed-in version of original image.
# EXPERIMENT: Take an image which is zoomed in or blurred and a prompt and a correct response and calculate it's implicit reward or divide the log-probability of the response from the trained model by the log-probability from the reference model.
...
def blur_image(image):
    ...
    blur_transform = v2.GaussianBlur(kernel_size=(19, 19), sigma=(3.0, 7.0))
    return blur_transform(image.squeeze(0)).unsqueeze(0)

# function to load and process an image from image path
...
def prepare_inputs(prompt, chosen, rejected, img_path, tokenizer, model):
    ...
    # # process the image into a tensor
    # image_tensor = model.process_images([image], model.config).to(dtype=model.dtype)
    batch["image"] = process_image(img_path, model)
...
cropped_image = crop_image(data['image'])
blurred_image = blur_image(data['image'])

# compute the log-probability on the mdpo model
...
mdpo_cropped_log_probs = compute_response_log_probs(mdpo_model, data['chosen_input_ids'], data['chosen_attention_mask'], data['chosen_labels'], cropped_image)
mdpo_blurred_log_probs = compute_response_log_probs(mdpo_model, data['chosen_input_ids'], data['chosen_attention_mask'], data['chosen_labels'], blurred_image)

# compute the log-probability on the reference model
...
reference_cropped_log_probs = compute_response_log_probs(reference_model, data['chosen_input_ids'], data['chosen_attention_mask'], data['chosen_labels'], cropped_image)
reference_blurred_log_probs = compute_response_log_probs(reference_model, data['chosen_input_ids'], data['chosen_attention_mask'], data['chosen_labels'], blurred_image)

print(f"{'mDPO/Reference Log-Probs:':<35} {mdpo_log_probs:8.4f}\t{reference_log_probs:8.4f}")
print(f"{'Cropped mDPO/Reference Log-Probs:':<35} {mdpo_cropped_log_probs:8.4f}\t{reference_cropped_log_probs:8.4f}")
print(f"{'Blurred mDPO/Reference Log-Probs:':<35} {mdpo_blurred_log_probs:8.4f}\t{reference_blurred_log_probs:8.4f}")