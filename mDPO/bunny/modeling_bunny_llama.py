...
from configuration_bunny_llama import SigLipVisionConfig
...
from configuration_bunny_llama import LlamaConfig
...
from configuration_bunny_llama import BunnyLlamaConfig
...
class mDPOBunnyLlamaForCausalLM(BunnyLlamaForCausalLM):
    def forward(
            self,
            input_ids: torch.LongTensor = None,
            attention_mask: Optional[torch.Tensor] = None,
    ...
    def crop_images(self, images):
        new_images = []
        for image in images:
            resize_cropper = v2.RandomResizedCrop(size=image.size()[-2:], scale=(0.01, 0.20))
    ...
    #         step = 50  # step at which we will pick the image
...
AutoModelForCausalLM.register(BunnyLlamaConfig, mDPOBunnyLlamaForCausalLM)